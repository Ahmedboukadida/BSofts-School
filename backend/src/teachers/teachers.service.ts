  ...
  async create(dto: CreateTeacherDto, user?: any) {
    ...
    if (dto.email && !userId) {
      ...
      if (existingUser) {
        ...
        const baseUsername = dto.email.split('@')[0].toLowerCase().replace(/[^a-z0-9]/g, '') || 'teacher';
        ...
        const tempPassword = (dto as any).password || `Tch_${Math.random().toString(36).slice(-8)}!${Math.floor(10 + Math.random() * 90)}`;
